...
export class TiposVeiculosFormComponent implements OnInit {
    ...
    this.route.params.subscribe(params => {
      if (params['id']) {
        ...
        this.tiposVeiculosService.obter(this.idEmEdicao).subscribe({
          ...
          error: () => { this.carregando = false; this.toastService.erro('Não foi possível carregar o tipo de veículo.', 'Erro'); }
        });
      }
    });
  }
  ...
  onSalvar(dados: any): void {
    ...
    requisicao.subscribe({
      next: () => {
        this.toastService.sucesso('Tipo de veículo salvo com sucesso.', 'Sucesso');
        this.router.navigate(['/tipos-veiculos']);
      },
      error: (erro) => this.toastService.erro(erro?.error?.message || 'Não foi possível salvar o tipo de veículo.', 'Erro')
    });
  }
